...
def vpi_laser_advanced(time_array, dt, current_array, I_bias, tuning_eff, thermal_mag, thermal_tau, linewidth):
    # Fixed the thermal sag so it physically droops the frequency
    ...
    inst_freq = (tuning_eff * dynamic_current) + thermal_sag 
...
# ==========================================
# 2. SIMULATION LOGIC (SPLIT INTO TWO PHASES)
# ==========================================
def calculate_predistortion(time_window, sweep_bw, iterations, gain, tau, thermal_mag, thermal_tau, 
    ...
    try:
        ...
        for iteration in range(iterations):
            ...
            ignore_idx = int(0.02 * len(measured_rf_freq))
            if ignore_idx > 0:
        ...
        lin_err_before = (freq_history[0] - ideal_target_freq) / 1e9
        lin_err_after = (freq_history[-1] - ideal_target_freq) / 1e9
        ...
        beat_err_before = (pd_freq_history[0] - ref_freq) / 1e6
        beat_err_after = (pd_freq_history[-1] - ref_freq) / 1e6
        ...
        ax_b1.plot(t * 1e6, freq_history[0] / 1e9, '#e74c3c', linewidth=2, label="Without Predistortion (Iter 0)")
        ax_b1.plot(t * 1e6, freq_history[-1] / 1e9, '#2ecc71', linewidth=2, label=f"Final Predistortion (Iter {iterations-1})")
        ...
        ax_b2.plot(t * 1e6, beat_err_before, '#e74c3c', linewidth=2, alpha=0.8, label="Error Without Predistortion")
        ax_b2.plot(t * 1e6, beat_err_after, '#2ecc71', linewidth=2, alpha=0.9, label="Error With Predistortion")
...
def run_closed_loop(time_window, tau, thermal_mag, thermal_tau, linewidth, ref_freq, pd_bandwidth, elec_delay, gain):
    ...
    try:
        ...
        ignore_idx = int(0.02 * len(measured_rf_freq_base))
        if ignore_idx > 0:
        ...
        I_integrator = gain * cumulative_trapezoid(filtered_error, dx=dt, initial=0)
        closed_loop_current = global_predistorted_ramp + I_integrator
        ...
        sim_data = {
            ...
            'Reference': {'sig': V_ref, 'freq': np.full_like(t, ref_freq)},
            'Mixer': {'sig': V_mixer},
            'LPF & Integrator': {'sig': I_integrator, 'freq': error_freq_base}
        }

        lin_err_base = (true_inst_freq_base - global_target_freq) / 1e9
        lin_err_cl = (true_inst_freq_cl - global_target_freq) / 1e9
        ...
        beat_err_base = error_freq_base / 1e6
        beat_err_cl = error_freq_cl / 1e6
        ...
        ax_c2.plot(t * 1e6, beat_err_base, '#e74c3c', linewidth=2, alpha=0.5, label="Residual Error (Open Loop)")
        ax_c2.plot(t * 1e6, beat_err_cl, '#2980b9', linewidth=2, alpha=0.9, label="Locked Error (Closed Loop)")
...
def plot_block_data(block_name):
    ...
    if block_name == 'Photodiode' and 'sig_before' in data:
        ...
        # Smoothed out the NFFT overlapping to remove the visual rippling 
...
def build_gui():
    ...
    # --- CORRECTED GAIN AND THERMAL SAG DEFAULTS ---
    time_var = tk.DoubleVar(value=1000.0)        
    bw_var = tk.DoubleVar(value=100.0)           
    iter_var = tk.IntVar(value=4)                
    gain_var = tk.DoubleVar(value=2.0e-6)        # Increased to aggressively bend the 10mA ramp
    tau_var = tk.DoubleVar(value=28.6)           
    thermal_mag_var = tk.DoubleVar(value=25.0)   # Fixed sign: Now causes frequency droop
